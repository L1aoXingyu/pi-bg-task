...
const glm = { ...model, id: "zai-org/GLM-5.3", provider: "siliconflow", api: "openai-completions" } as Model<Api>;
const claude = { ...model, id: "claude-sonnet-4-5", provider: "anthropic", api: "anthropic-messages", promptCache: { short: 300, long: 3600 } } as Model<Api>;
...
const completionsPayload = () => ({model: glm.id, stream: true, messages: [{role: "system", content: "sys"}, {role: "user", content: "private fixture"}], max_tokens: 8192, tools: [{type: "function", name: "dangerous", parameters: {type:"object"}}]});
const anthropicPayload = () => ({model: claude.id, messages: [{role: "user", content: [{type: "text", text: "private fixture"}]}], max_tokens: 8192, system: [{type: "text", text: "sys", cache_control: {type: "ephemeral"}}]});
...
const result = (u = usage(), text = "OK", stopReason = "stop", m = model) => ({role: "assistant", provider: m.provider, model: m.id, api: m.api, content: [{type: "text", text}], usage: u, stopReason, timestamp: 1});
const flush = () => new Promise<void>(resolve => setImmediate(resolve));

function harness(saved: any[] = [], current = model, thinkingLevel?: string) {
	let now = 1_000_000;
	let seq = 0;
	...
	const state = {jobs: ["job1"], blocked: false, idle: true, pending: false, model: structuredClone(current), response: result(usage(), "OK", "stop", current), slow: false, reconciles: 0};
	...
	const ctx: any = {hasUI: true, get model() {return state.model;}, get thinkingLevel() {return thinkingLevel;}, isIdle: () => state.idle, hasPendingMessages: () => state.pending,
		...
		modelRegistry: { getProvider() { return {}; }, streamSimple(_model: any, _context: any, options: any) {
			const generated = Array.isArray(payload().input) && current.api !== "openai-completions" && current.api !== "anthropic-messages"
				? {input: [{role:"assistant",content:[{type:"output_text",text:"READY"}]}]}
				: {messages: [{role:"assistant", content: "READY"}]};
			const sent = options.onPayload(generated);
	...
	async function ready(body: unknown = payload()) {
		await emit("before_provider_request", {payload: body});
		const message = result(usage(), "OK", "stop", current);
		entries.push({type:"message", id:`a${entries.length}`, message});
...
test("reject delta/background/store payloads; all providers with a model id are eligible", () => {
	...
	assert.equal(eligibleModel(grok), true);
	assert.equal(eligibleModel(glm), true);
	assert.equal(eligibleModel(undefined), false);
	assert.deepEqual(capturePayload(payload(), model), payload());
	assert.equal(capturePayload({...payload(), prompt_cache_key:undefined}, model)?.model, model.id);
	for (const p of [{...payload(), previous_response_id:"id"}, {...payload(), store:true}, {...payload(), background:true}, {...payload(), input:"bad", messages:undefined}, {...payload(), model:"other"}]) assert.equal(capturePayload(p,model), undefined);
});
...
test("default on; waits for a fresh request; 25 minute schedule", async () => {
	const h=harness(); await h.emit("session_start"); assert.equal(h.warm.status().enabled,true); assert.equal(h.timers.size,0);
	await h.ready();
	...
	assert.equal(h.entries.filter(e=>e.type==="message").length,1); // only the real request
});
...
test("native warming is suppressed only with a live job-aware snapshot",async()=>{
	const h=harness();await h.emit("session_start");
	assert.equal(await h.emit("cache_warming_decision"),undefined);
	await h.ready();
...
test("completions payloads append a string user turn and cap max_tokens", () => {
	const p = completionsPayload(); const copy = structuredClone(p);
	const warm = refreshPayload(p, [{role:"assistant", content:"READY"}]);
	assert.deepEqual(p, copy);
	assert.deepEqual(warm.messages!.slice(0,-2), p.messages);
	assert.equal(warm.max_tokens, 32);
	assert.match(JSON.stringify(warm.messages!.at(-1)), /Reply exactly OK/);
	assert.deepEqual(capturePayload(p, glm)?.messages, p.messages);
});

test("xAI responses and SiliconFlow completions schedule without /bg-warm on", async () => {
	for (const [current, body] of [[grok, {...payload(), model: grok.id}], [glm, completionsPayload()]] as const) {
		const h=harness([], current); await h.emit("session_start"); await h.ready(body);
		await h.advance(); assert.equal(h.calls.length,1); assert.equal(h.calls[0].sent.model, current.id);
	}
});

test("Anthropic thinking is not replayed unless adaptive thinking is forced", () => {
	assert.equal(replaySafe(claude, "off"), true);
	assert.equal(replaySafe(claude, "high"), false);
	assert.equal(replaySafe({...claude, compat:{forceAdaptiveThinking:true}}, "high"), true);
	assert.equal(replaySafe(model, "high"), true);
});

test("known short cache TTL tightens the refresh cadence", () => {
	assert.equal(refreshIntervalMs(model), P.intervalMs);
	assert.equal(refreshIntervalMs(claude), 270_000);
	assert.equal(refreshLimit(model), 8);
	assert.ok(refreshLimit(claude) > 8);
});

test("Anthropic thinking-on requests are not captured", async () => {
	const h=harness([], claude, "high"); await h.emit("session_start"); await h.ready(anthropicPayload());
	await h.advance(270_000); assert.equal(h.calls.length,0);
});
